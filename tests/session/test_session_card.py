"""Tests for ocd.session.session_card module."""
...
class TestBuildSessionCard:
    ...
    def test_empty_state(self, mock_config_paths):
        card = ocd.session.session_card.build_session_card()
        assert card == ""

    def test_with_manifest(self, mock_config_paths):
        ocd.session.session_card.MANIFEST_FILE.write_text(json.dumps({"version": 1, "agents": []}))
        card = ocd.session.session_card.build_session_card()
    ...
    def test_with_kb_index(self, mock_config_paths):
        ...
        ocd.session.session_card.KB_INDEX_JSON.write_text(json.dumps(index))
        card = ocd.session.session_card.build_session_card()
...
class TestUpdateSessionCard:
    ...
    def test_creates_card_file(self, mock_config_paths):
        sf = ocd.session.session_card.SESSION_CARD_FILE
        assert not sf.exists()
        ocd.session.session_card.update_session_card("src/foo.py")
        assert sf.exists()

    def test_appends_entry(self, mock_config_paths):
        sf = ocd.session.session_card.SESSION_CARD_FILE
        ocd.session.session_card.update_session_card("src/foo.py", action="edit")
        ocd.session.session_card.update_session_card("src/bar.py", action="edit")
    ...
    def test_default_action_is_edit(self, mock_config_paths):
        sf = ocd.session.session_card.SESSION_CARD_FILE
        ocd.session.session_card.update_session_card("src/baz.py")
        content = sf.read_text()
        assert "edit: src/baz.py" in content

    def test_fifo_eviction(self, mock_config_paths):
        max_chars = ocd.session.session_card.MAX_SESSION_CARD_CHARS
        sf = ocd.session.session_card.SESSION_CARD_FILE

        for i in range(100):
            ocd.session.session_card.update_session_card(f"src/file_{i:03d}.py")
        content = sf.read_text()
    ...
    def test_timestamp_format(self, mock_config_paths):
        sf = ocd.session.session_card.SESSION_CARD_FILE
        ocd.session.session_card.update_session_card("src/timed.py")
        content = sf.read_text()
...
class TestLoadSessionCard:
    ...
    def test_returns_none_when_missing(self, mock_config_paths):
        result = ocd.session.session_card.load_session_card()
        assert result is None

    def test_returns_none_when_empty(self, mock_config_paths):
        ocd.session.session_card.SESSION_CARD_FILE.write_text("")
        result = ocd.session.session_card.load_session_card()
        assert result is None
